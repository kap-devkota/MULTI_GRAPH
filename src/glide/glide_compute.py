...
def compute_X_normalized(A, t=-1, lm=1, is_normalized=True):
    ...
    # Add small positive where the degree is zero
    d = np.where(d == 0.0, 0.0001, d)
...
def glide_predict_links(edgelist, X, params={}, thres_p=0.9):
    ...
    if thres_p > 0:
        ...
        glide_mat[i, j] = np.where(glide_mat >= thres, 1.0, 0.0)
    return glide_mat
...
def glide_compute_map(pos_df, thres_p=-1, params={}):
    """
    ...
    if isinstance(pos_df, pd.DataFrame):
        ...
        assert isinstance(pos_df, tuple) and len(pos_df) == 2
        A, glide_map = pos_df
        u_nzero      = np.nonzero(A)
        u_vals       = A[u_nzero]
        u_ed         = np.concatenate([np.transpose(u_nzero),
                                      u_vals.reshape(-1, 1)], axis = 1)
        u_edges      = [(int(np.round(p)), int(np.round(q)), v) for (p, q, v) in list(u_ed)]
        
        
    X = compute_X_normalized(
        A, lm=params["lam"], is_normalized=params["norm"]
    )
