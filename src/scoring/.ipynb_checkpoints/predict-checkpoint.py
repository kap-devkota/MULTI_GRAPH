...
def vote(voters, labels_f, weight_f, confidence = True):
    """
    ...
    max_vote  = max([label_counts[k] for k in label_counts])
    if confidence:
        return [(key, label_counts[key]/max_vote) for key in label_counts]
    return max_label
    

def wmv(A, labels_f, weight_f=lambda x: x, default_label="????", confidence = True, params = None):
    """
    ...
    for i in range(n):
        ...
        if filter_type == "degree":
            ...
            voters = voters_
        prediction = vote(voters, labels_f, lambda voter: weight_f(A[i, voter]), confidence = confidence)
...
def mv(A, labels_f, confidence = True, default_label="????"):
    """
    ...
    Output:
      ...
      is already known, the first label in the list is picked.
    """
    return wmv(A, labels_f, weight_f=lambda _: 1, default_label=default_label, confidence = confidence)


def glide(node_association, 
          labels_f, 
          confidence = True):
    """
    Function that performs prediction for test proteins.
    node_association : map protein -> list[protein]. A map of protein that returns the list of proteins
    labels_function: lambda protein -> list [GO labels]
    """
    ...
    for i in range(n):
        ...
        prediction = vote(voters, labels_f, weight_f, confidence)
        if prediction is not None:
            predicted_labels[i] = prediction
        else:
            predicted_labels[i] = []
